  ...
  const loadData = async (id: string) => {
    ...
    try {
      ...
      if (e.message && e.message.includes("DEPLOYMENT_CONFIG_ERROR")) {
        setErrorMessage("Configuration Error: The Google Script URL is invalid or malformed.");
      } else if (e.message && e.message.includes("INVALID_RESPONSE")) {
        setErrorMessage("Data Error: The Google Script returned an invalid response.");
    ...
    if (loadingState === LoadingState.IDLE) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 animate-fade-in">
          ...
          <p className="mt-8 text-xs text-slate-400">© {new Date().getFullYear()} ISO FM Academy. All rights reserved.</p>
        </div>
      );
    }
        ...
        <div className="max-w-lg mx-auto mt-20 px-4 text-center animate-fade-in">
          <div className="bg-red-50 p-8 rounded-2xl border border-red-100 shadow-sm">
            ...
            <div className="text-red-700 mb-6 text-sm leading-relaxed">
              {isPermissionError ? (
                <div className="text-left bg-white p-4 rounded-lg border border-red-200 shadow-sm text-xs">
                  <p className="font-bold mb-2">The Google Script is not accessible. To fix this:</p>
                  <ol className="list-decimal pl-4 space-y-1">
                    <li>Go to your Apps Script editor.</li>
                    <li>Click Deploy > Manage Deployments > Edit.</li>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => { setLoadingState(LoadingState.IDLE); window.location.hash = ''; }} className="w-full px-4 py-3 bg-white text-red-700 font-bold rounded-xl border border-red-200">Return to Login</button>
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="animate-fade-in pb-12 print:pb-0">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-sm text-slate-500 mt-2">
                   ...
                   <a href={`mailto:${record.respondentEmail}`} className="flex items-center gap-1.5 hover:text-blue-600 transition-colors print:text-slate-900">
                    <Mail className="w-4 h-4" /> {record.respondentEmail}
                  </a>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <div className="lg:col-span-1"><ScoreGauge score={record.aiMaturityScore} level={record.aiMaturityLevel} /></div>
