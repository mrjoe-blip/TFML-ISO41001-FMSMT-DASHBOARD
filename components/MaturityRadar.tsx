...
export const ClauseAnalysisChart: React.FC<ClauseChartProps> = ({ data }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const statusColor = score >= 70 ? 'text-blue-300' : score >= 40 ? 'text-amber-300' : 'text-red-300';
      
      return (
        <div className="bg-slate-900/95 backdrop-blur text-white p-4 rounded-xl shadow-2xl border border-slate-700 min-w-[150px]">
          <p className="font-bold text-sm mb-2 border-b border-slate-700 pb-2">{payload[0].payload.name} <span className="text-slate-400 font-normal">({payload[0].payload.code})</span></p>
          <div className="flex justify-between items-end">
            <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Score</span>
            <span className={`text-xl font-mono font-bold ${statusColor}`}>{score}/100</span>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="w-full h-[400px] bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400 text-center">Performance across key standards</p>
      </div>
